...
const params = new URLSearchParams({
    device_id: navigation.pathInfo.deviceId || '',
    date: navigation.pathInfo.date || '',
    file_id: item.id,
    file_name: item.name,
    status: navigation.pathInfo.status || '',
})
navigate(`/annotation/labeling?${params.toString()}`)
```
...
elevation: 0：无阴影，和屏幕平齐
elevation: 3：轻微阴影（常用卡片、按钮）
elevation: 6：中等阴影（弹窗、顶部导航）
elevation: 24：最大阴影（模态框、浮层）
...
```
git worktree remove D:\code\react-vision-annotator-agent-a7f93d
```

如果删除不掉， 强行删除

```
git worktree remove -f D:\code\react-vision-annotator-agent-a7f93d
```
...
```
强制删除分支（不在乎是否残留未合并的代码）
```
git branch -D feat-force
```

刷新远程分支列表

如果不运行这个的话， 那么远程分支的列表是不会更新的
...
这些配置会在保存文件时自动执行代码操作，包括：
source.fixAll：自动修复代码问题（包含删除未使用的变量）
source.organizeImports：自动整理导入语句
source.sortMembers：自动排序类 / 对象成员

如果你想关闭「自动删除未使用变量」的功能
有两个方案可选：
方案 1（推荐）：只删除 fixAll，保留整理导入与排序
...
修改完成后保存文件即可立即生效。
需要我再帮你把这份配置适配成 React / React Native 专用的最佳格式化规则吗？